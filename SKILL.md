...
6. Сборку не запускай, если не просили. Дев-сервер обычно уже поднят человеком: `curl -s localhost:9080/<модуль>/-/web.audit.js` пересоберёт модуль и отдаст аудит, чини всё. Нет ответа на 9080 — попроси человека запустить `npm start` у себя. Сам поднимай, только если человека рядом нет, и только так, в фоне:

   ```bash
   sleep 100000 | npx mam
   ```

   Без открытого stdin сервер печатает `Started` и молча выходит с кодом 0.
