...
Пишите значение строкой — так короче и читается без импорта:
...
Хелперы `$mol_style_unit` (`rem`, `px`, `per`) остались и работают, но в новом
коде их уже не пишут: `rem(50)` — старая форма, встречается в примерах постарше.
Они по-прежнему полезны, когда единицу надо посчитать, а не записать:
...
- В сравнениях пишите `undefined`, а не `void 0`: `void 0` — наследие старого кода, специально его воспроизводить не нужно
