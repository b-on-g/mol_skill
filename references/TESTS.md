...
**Сценарий пользователя.** Вид в $mol это функция состояния, поэтому сценарий пишется через методы вида, без селекторов и без браузера. Обязателен на каждую фичу.
...
Подмена через `$` достаёт только вызовы `this.$.X`. Глобалы (`fetch`, `setTimeout`, `Date`) подменять на `globalThis`. Таймеры в тестах замоканы, время прокручивается `$mol_after_mock_warp()`.
