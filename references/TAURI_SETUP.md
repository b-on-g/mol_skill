...
Без пути MAM поднимает дев-сервер и пересобирает модули по запросу. Приложение будет доступно на `http://localhost:9080/bog/quiz/`. С путём (`npm start bog/quiz`) сервер не поднимается: модуль собирается один раз, и команда завершается.
...
Команда собирает бандлы в `bog/quiz/-/` и завершается сама.
